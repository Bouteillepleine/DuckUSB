...
**Automatic in every scoped non-core app**, with no toggle. Property reads are process-local, so no `system_server` hook can reach them; this half only works in apps you scope.
